...
We only collect data we actually need to operate the Service, fulfil your orders, comply with the law, and improve Flowva. The categories below describe what we collect and how we get it.

### 2.1 Account data
When you create a Flowva account, we collect:
- Your name (and any display name you choose);
- Your e-mail address;
- A securely hashed password (we never see or store your password in plain text);
- Account preferences and settings.

### 2.2 Order and delivery data
When you place an order, we process:
- The products you order and your order history;
- Your delivery address (recipient name, street, postal code, city, country);
- Your phone number (required by carriers and customs for delivery and import clearance);
- Quality-control records linked to your order, including inspection results and measurement photos of the item before international shipping;
- Returns, withdrawals, complaints, and after-sales records.
...
- A device identifier and browser/technical information needed to keep you signed in and to operate the app as a Progressive Web App (PWA);
- If you opt in to web-push notifications, a push subscription token and the related browser endpoint, so we can send you order-status updates;
- Basic technical logs (for example, IP address, timestamps, error and security logs) generated automatically by our hosting infrastructure.

### 2.6 Support and communication data
When you contact us or use in-app support, we process:
- The content of your messages, support tickets, and after-sales requests;
- Any attachments, photos, or details you provide to resolve a problem;
...
Under the GDPR we must have a lawful basis for every purpose. The table below sets out each purpose, the data involved, and the lawful basis.
...
We do not sell your personal data. We share it only with the parties below, each of which acts as a **processor** on our instructions (or as an independent controller where stated), under a data-processing agreement and only to the extent necessary.
...
- To **carriers and customs authorities** as required to deliver your order and clear it through import (DDP);
- To **professional advisors** (accountants, lawyers) under confidentiality;
...
`[LEGAL REVIEW: Confirm specific retention periods against the Dutch Civil Code, the Dutch Tax Act (7-year administration obligation), consumer-law limitation periods for conformity/warranty claims, and any product-liability limitation period. Adjust the figures above to the final agreed schedule.]`
...
- **Right to erasure ("right to be forgotten")** — have your data deleted where there is no overriding reason for us to keep it (note: we may need to retain some data to meet legal obligations, e.g. tax records).
- **Right to restriction** — ask us to limit processing in certain situations, for example while a dispute about accuracy is resolved.
...
- **Right to withdraw consent** — where we rely on consent (e.g. push notifications, marketing), withdraw it at any time. This does not affect processing carried out before withdrawal.
- **Rights regarding automated decision-making** — we do not make decisions producing legal or similarly significant effects about you based solely on automated processing.

We may need to verify your identity before acting on a request, to protect your data.

### Right to complain
If you believe we have not handled your data correctly, please contact us first so we can try to resolve it. You also have the right to lodge a complaint with the Dutch data-protection authority:

- **Autoriteit Persoonsgegevens (AP)** — the Dutch Data Protection Authority. Website: autoriteitpersoonsgegevens.nl.
...
- Encryption of data in transit (HTTPS/TLS) and protection of data at rest in our hosting environment;
- Hashed passwords — we never store passwords in plain text;
- Card data handled by Stripe under PCI-DSS standards — we do not store full card numbers;
